...
- The README contradicted itself: the feature list still claimed there was "no
  transport configured that could deliver them", while the section below it
  described how to deliver captured mail. True until 0.2.x, wrong since resending
  exists.
- `mailcatcher:resend` was missing from the command reference, and the
  `mailcatcher:prune` example omitted the `--force` that a Production context
  requires.

### Removed

- Label `action.show`, unused since the list started linking the subject itself.
